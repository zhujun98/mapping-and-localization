...
void ParticleFilter::updateWeight(const std::vector<Observation> &observations, const Map &map) {
  ...
  for (size_t i=0; i<particles_.size(); ++i) {
    ...
    for (size_t j=0; j<map.getLandmarkList().size(); ++j) {
      Observation observation;

      // just copy
      observation.id = map.getLandmarkList()[j].id;
      observation.x = map.getLandmarkList()[j].x;
      observation.y = map.getLandmarkList()[j].y;
